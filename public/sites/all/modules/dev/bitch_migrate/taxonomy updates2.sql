...
INSERT INTO field_revision_field_general_topic SELECT
  'node'    AS entity_type,
  'article' AS bundle,
  0         AS deleted,
  entity_id,
...
/* Join in the tag map such that the original tag on the tagname associated with the original node */
                LEFT JOIN bitch_d7.taxonomy_term_data ttd ON ttd.name = tm.subtopic AND ttd.vid = 6
              WHERE td.vid = 2
...
INSERT INTO field_data_field_general_topic SELECT
                                                 'node'    AS entity_type,
                                                 'article' AS bundle,
                                                 0         AS deleted,
                                                 entity_id,
